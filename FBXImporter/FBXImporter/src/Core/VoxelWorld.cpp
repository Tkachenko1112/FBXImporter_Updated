...
namespace RayTracing {
    ...
    inline bool AnyHit(std::vector<Triangle>& triangles, glm::vec3 rayOrign, glm::vec3 rayDir, float minDist, float maxDist) {
        ...
        for (Triangle& tri : triangles) {
            ...
            // Cast ray
            IntersectionResult result = RayTriangleIntersectTest(tri.p1, tri.p2, tri.p3, rayOrign, rayDir);
...
void VoxelWorld::GenerateTriangleOccluders() {
    ...
    for (Triangle& tri : _staticWorldTriangles) {
        if (_optimizeHack) {
            if (GetMaxXPointOfTri(tri) <= _voxelSize ||
                GetMaxYPointOfTri(tri) <= _voxelSize ||
                GetMaxZPointOfTri(tri) <= _voxelSize ||
                GetMinXPointOfTri(tri) >= (MAP_WIDTH - 1) * _voxelSize - _voxelSize ||
                GetMinYPointOfTri(tri) >= (MAP_HEIGHT - 1) * _voxelSize - _voxelSize ||
                GetMinZPointOfTri(tri) >= (MAP_DEPTH - 1) * _voxelSize - _voxelSize) {
                continue;
            }
        }
    ...
    for (Triangle& tri : _triangles) {
        tri.normal = NormalFromTriangle(tri);
    }
                    ...
                    //Light light = VoxelWorld::GetLightByIndex(0);
                        ...
                        if (CellIsEmpty(x + 1, y, z)) {
                            ...
                            if (!ClosestHit(origin, destination, offset).hitFound) {
                                voxel.forwardFaceX.accumulatedDirectLighting += GetDirectLightAtAPoint(light, (origin - NRM_X_FORWARD) * _voxelSize, NRM_X_FORWARD, _voxelSize);
                            }
                        }                        
                        ...
                        if (CellIsEmpty(x - 1, y, z)) {
                            ...
                            if (!ClosestHit(origin, destination, offset).hitFound) {
                                voxel.backFaceX.accumulatedDirectLighting += GetDirectLightAtAPoint(light, (origin - NRM_X_BACK) * _voxelSize, NRM_X_BACK, _voxelSize);
                            }
                        }
                        ...
                        if (CellIsEmpty(x , y, z + 1)) {
                            ...
                            if (!ClosestHit(origin, destination, offset).hitFound) {
                                voxel.forwardFaceZ.accumulatedDirectLighting += GetDirectLightAtAPoint(light, (origin - NRM_Z_FORWARD) * _voxelSize, NRM_Z_FORWARD, _voxelSize);
                            }
                        }
                        ...
                        if (CellIsEmpty(x , y, z - 1)) {
                            ...
                            if (!ClosestHit(origin, destination, offset).hitFound) {
                                voxel.backFaceZ.accumulatedDirectLighting += GetDirectLightAtAPoint(light, (origin - NRM_Z_BACK) * _voxelSize, NRM_Z_BACK, _voxelSize);
                            }
                        }
                        ...
                        if (CellIsEmpty(x, y + 1, z)) {
                            ...
                            if (!ClosestHit(origin, destination, offset).hitFound) {
                                voxel.YUpFace.accumulatedDirectLighting += GetDirectLightAtAPoint(light, (origin - NRM_Y_UP) * _voxelSize, NRM_Y_UP, _voxelSize);
                            }
                        }
                        ...
                        if (CellIsEmpty(x, y - 1, z)) {
                            ...
                            if (!ClosestHit(origin, destination, offset).hitFound) {
                                voxel.YDownFace.accumulatedDirectLighting += GetDirectLightAtAPoint(light, (origin - NRM_Y_DOWN) * _voxelSize, NRM_Y_DOWN, _voxelSize);
                            }
                        }
                    }
                }
...
void ApplyLightToProbe(VoxelFace& voxelFace, GridProbe& probe) {
    ...
    if (!RayTracing::AnyHit(_triangles, origin, rayDirection, MIN_RAY_DIST, distanceToProbe)) {
...
GridProbe& GetRandomGridProbe() {
    int x = RandomInt(0, PROPOGATION_WIDTH - 1);
    int y = RandomInt(0, PROPOGATION_HEIGHT - 1);
    int z = RandomInt(0, PROPOGATION_DEPTH - 1);
    return _propogrationGrid[x][y][z];
}

VoxelFace& GetRandomVoxelFaceXForward() {
    int i = RandomInt(0, _voxelFacesXForward.size() - 1);
    return _voxelFacesXForward[i];
}

VoxelFace& GetRandomVoxelFaceXBack() {
    int i = RandomInt(0, _voxelFacesXBack.size() - 1);
    return _voxelFacesXBack[i];
}

VoxelFace& GetRandomVoxelFaceYUp() {
    int i = RandomInt(0, _voxelFacesYUp.size() - 1);
    return _voxelFacesYUp[i];
}

VoxelFace& GetRandomVoxelFaceYDown() {
    int i = RandomInt(0, _voxelFacesYDown.size() - 1);
    return _voxelFacesYDown[i];
}

VoxelFace& GetRandomVoxelFaceZForward() {
    int i = RandomInt(0, _voxelFacesZForward.size() - 1);
    return _voxelFacesZForward[i];
}

VoxelFace& GetRandomVoxelFaceZBack() {
    int i = RandomInt(0, _voxelFacesZBack.size() - 1);
